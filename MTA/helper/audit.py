...
def set_dtypes_and_nan(df, df_info, missing_val_codes, dtype_dict, cols_known_to_keep=[]):
    ...
    if df['src_subject_id'].iloc[0] == 'Subject ID how it\'s defined in lab/project':
        df = df.drop(0, axis=0)  # Drop the first row if it contains a description
    ...
    if 'interview_date' in df.columns:
        subject_unspecific_cols.append('interview_date')  # Ensure interview_date is considered subject-unspecific
    ...
    df_clean = df.drop(columns=subject_unspecific_cols, errors='ignore')  # Remove subject-unspecific columns
    df_clean = df_clean.replace(np.nan, -999)  # Replace NaN values with placeholder (-999)
    ...
    for col in df_clean.columns:
        if col in df_info['ElementName'].tolist():
            try:
                dtype = dtype_dict[df_info.loc[df_info['ElementName'] == col, 'DataType'].values[0]]
                df_clean[col] = df_clean[col].astype(dtype)  # Convert column to the specified dtype
            except (IndexError, ValueError, TypeError) as e:
                print(f"Error converting column {col}: {e}")  # Handle dtype conversion errors
        elif col in cols_known_to_keep and col not in df_info['ElementName'].tolist():
            print(f"Skipping type conversion for column {col} as it's not in the info dictionary.")

    # Replace missing value codes back to NaN
    df_clean = df_clean.replace([-999, "-999"], np.nan).copy()  # Restore NaNs

    return df_clean


def remove_cols(df, cols_known_to_remove=[], cols_known_to_keep=[], thr_drop_missing=50):
    df_clean = remove_nan_cols(df, cols_known_to_keep)  # Remove columns that are entirely NaN
    df_clean = remove_const_cols(df_clean, cols_known_to_keep)  # Remove columns with constant values
    df_clean = remove_raw_and_know_cols(df_clean, cols_known_to_remove, cols_known_to_keep)  # Remove predefined raw/known columns
    df_clean = remove_thr_empty_cols(df_clean, thr_drop_missing, cols_known_to_keep)  # Remove columns exceeding missing threshold
    return df_clean


def remove_nan_cols(df, cols_known_to_keep):
    empty_cols = df.columns[(df.isna().all(axis=0)) & (~df.columns.isin(cols_known_to_keep))]  # Identify fully missing columns
    print("Removing empty columns ..  N = ", len(empty_cols))
    print(empty_cols)
    df = df.drop(columns=empty_cols)  # Drop fully missing columns
    print(df.shape)
    return df
...
def remove_const_cols(df_clean, cols_known_to_keep):
    ...
    df_clean = df_clean.drop(columns=constant_cols)  # Drop columns with only one unique value
    print(df_clean.shape)
    return df_clean


def remove_raw_and_know_cols(df_clean, cols_known_to_remove, cols_known_to_keep):
    cols2rem = find_mean_raw_vs_t_cols(df_clean, cols_known_to_remove, cols_known_to_keep)  # Find mean/raw columns to remove
    print("Removing known and raw columns..  N =  :", len(cols2rem))
    print(cols2rem)

    try:
        df_clean = df_clean.drop(columns=cols2rem)  # Drop identified columns
        print(df_clean.shape)
    except KeyError as e:
        print('    ERROR')
        print(e)
        print("Removing non-existing columns from list .. ")
        cols2rem = [col for col in cols2rem if col in df_clean.columns]  # Keep only existing columns
        df_clean = df_clean.drop(columns=cols2rem)  # Drop them again after filtering
...
def remove_thr_empty_cols(df_clean, thr_drop_missing, cols_known_to_keep):
    missing_perc = df_clean.isna().mean(axis=0) * 100  # Calculate percentage of missing values per column
    cols_drop_miss = set(df_clean.columns[np.where(missing_perc > thr_drop_missing)])  # Identify columns exceeding threshold
    cols_drop_miss -= set(cols_known_to_keep)  # Ensure important columns are not removed

    print("Removing above threshold empty columns.. N =  :", len(cols_drop_miss))
    print(cols_drop_miss)
    df_clean = df_clean.drop(columns=cols_drop_miss)  # Drop high-missingness columns
    print(df_clean.shape)
    return df_clean


### Add a way to select specific columns where NaN conversion does not happen
def pre_audit(df, df_info, missing_val_codes, dtype_dict, cols_known_to_remove=None, cols_known_to_keep=[], thr_drop_missing=20):
    ...
    df_clean = df.replace(missing_val_codes, np.nan).copy()  # Convert missing value codes to NaN
    df_clean = set_dtypes_and_nan(df_clean, df_info, missing_val_codes, dtype_dict, cols_known_to_keep)  # Set dtypes and clean NaNs
    df_clean = remove_cols(df_clean, cols_known_to_remove, cols_known_to_keep, thr_drop_missing)  # Apply column removal rules

    print('Old shape: ', df.shape)
    print('New shape: ', df_clean.shape)

    verifiy_pre_audit(df_clean, missing_val_codes)  # Verify cleaning process
...
def verifiy_pre_audit(df_clean, missing_val_codes):
    ...
    print('Remaining, ends with x : ', x_columns)  # Check if 'x' suffix columns remain
    t_columns = [col for col in df_clean.columns if col.endswith('t')]
    print('Remaining, ends with t : ', t_columns)  # Check if 't' suffix columns remain
    raw_columns = [col for col in df_clean.columns if col.endswith('raw')]
    print('Remaining, ends with raw : ', raw_columns)  # Check if 'raw' suffix columns remain

    if df_clean.shape[1] <= 3:
        print('    WARNING! This dataframe is likely unusable.')  # Warn if too few columns remain
    ...
    if exists:
        ...
        print('    WARNING ! The following missing values have not been converted to NaN', still_present)  # Warn if missing values persist
